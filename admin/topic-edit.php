...
$topics = $ddf_db->fetch_all($ddf_db->topics);
...
$topics = $ddf_db->fetch_all($ddf_db->topics, "*", '', 'topicID DESC', $limit, $offset);
?>
		...
		<?php else : ?>
			...
			<?php foreach ($topics as $topic) : ?>
				...
				<tr id="entry-<?php echo $topic->topicID; ?>">
					...
					<td class="count-column"><?php echo $ddf_db->get_forum('forum_name', $topic->forumID); ?></td>

					<td class="count-column"><?php echo $topic->topic_replies; ?></td>

					<td><?php echo $user->get_user("username", $topic->topic_poster); ?></td>

					<td><?php echo time2str(timestamp($topic->topic_date)); ?></td>

					<td><?php echo time2str(timestamp($topic->topic_last_post_date)); ?></td>

					<td class="actions">
						<a class="action-edit" href="topic.php?action=edit&amp;id=<?php echo $topic->topicID; ?>"><span class="genericon genericon-edit"></span></a>
						
						<a class="action-view" href="<?php echo home_url(); ?>/topic.php?id=<?php echo $topic->topicID; ?>"><span class="genericon genericon-show"></span></a>
						
						<a class="action-delete" href="topic.php?action=delete&amp;id=<?php echo $topic->topicID; ?>"><span class="genericon genericon-close"></span></a>
					</td>
				</tr>
